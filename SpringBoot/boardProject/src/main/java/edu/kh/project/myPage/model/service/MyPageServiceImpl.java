...
public class MyPageServiceImpl implements MyPageService {
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		 ...
		 * 
		 * 3) 만약에 파일 저장 실패 시 -> 예외 발생 -> @Transactional 을 이용해서 rollback 수행
		 * 
		 */
...
//		uf2.setFileRename(fileRename);
	...
	// 프로필 이미지 변경 서비스
	@Override
	public int profile(MultipartFile profileImg, int memberNo) throws Exception {
		...
		Member mem = Member.builder().memberNo(memberNo).profileImg(updatePath).build();
